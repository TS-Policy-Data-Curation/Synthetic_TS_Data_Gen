...
from .eval_VAE_quality import load_timeVAE_data, evaluate_synthetic_data
...
def main():
    ...
    real = os.path.join(data_directory, 'inflation.csv')
    synth = real = os.path.join(data_directory, 'inflation.csv')
    real_features, synthetic_features, feature_columns = load_timeVAE_data(real, synth)
    evaluate_synthetic_data(real_features, synthetic_features, feature_columns)
